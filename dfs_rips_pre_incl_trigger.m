...
epochfilter{4} = ['isequal($cond_phase,''plateau'') & (isequal($environment,''goal'') | isequal($environment,''hybrid2'') | isequal($environment,''hybrid3'')) '];
...
f = setfilterfunction(f, 'dfa_ripquantpertrial_allphase_NF', {'ca1rippleskons','trials','pos'}, 'excltrigger',0);
f = runfilter(f);

%save('/media/anna/whirlwindtemp2/ffresults/NFrips_noCtrls_allphases.mat','f','-v7.3')
load('/media/anna/whirlwindtemp2/ffresults/NFrips_noCtrls_allphases.mat','f')

%% simulate expected time of trial given prevalence curves and size of trigger 
...
for a = 1:length(animals)
    ...
    earlydays = unique(f(a).epochs{2}(:,1));
    predata = [f(a).output{2}.rw,]; %f(a).output{2}(f(a).epochs{2}(:,1)<=earlydays(4)).rw
    %for e = 1:length(f(a).output{1}.rw)
     %   counts{a}(e,:) = mean(cell2mat(cellfun(@(x) histcounts(x.size,edges)',f(a).output{1}(e).rw,'un',0)));
    presizes = cell2mat(cellfun(@(x) x.size',predata,'UniformOutput',0));
    totprelength = sum(cellfun(@(x) x.duration,predata));
    precounts(a,:) = histcounts(presizes,edges);
    valprevs = find(precounts(a,:)>10);   % only compute prevalence if 10+ of these rips happened
    preprev(a,:) = precounts(a,:)./totprelength;
    cumprev(a,:) = cumsum(preprev(a,:),'reverse'); % prevalence of a rip this size or larger
    
    % calculate realdur and sim dur for early and late conditioning f(a).output{2}(f(a).epochs{2}(:,1)>earlydays(4)).rw,
    all = [f(a).output{3}.rw, f(a).output{4}.rw]; %f(a).output{2}.rw, f(a).output{1}.rw, 
    hasrips = cellfun(@(x) ~isempty(x.size),all);
    type = cellfun(@(x) x.type,all);
    realdur = cellfun(@(x) x.duration,all(type==1 & hasrips));
    trigsizes = cellfun(@(x) x.size(end),all(type==1 & hasrips));
    binnedtrigs{a} = [discretize(trigsizes,edges)];
    valsizes = ismember(binnedtrigs{a},valprevs);
    simdur = 1./cumprev(a,binnedtrigs{a}(valsizes));
    
    realsmooth = smooth(padarray(realdur(valsizes)', smoothingwin, 'replicate'),smoothingwin);
    simsmooth = smooth(padarray(simdur', smoothingwin, 'replicate'),smoothingwin);
    plot(realsmooth(smoothingwin:end-smoothingwin),'Color',animcol(a+4,:),'LineWidth',1); %subplot(1,2,1); hold on; 
    plot(simsmooth(smoothingwin:end-smoothingwin),':','Color',animcol(a+4,:),'LineWidth',1); %subplot(1,2,2); hold on; 
 end
title('sim based on pre+earlyNF days; predict lateNF&stableNF; 200trialsmooth' )
ylabel('pre-reward duration (s)'); xlabel('trial')

%% plot trigger size (last rip on r trials) by phase (second half of ep only)
clearvars -except f animals
animcol = [254 123 123; 255 82 82; 255 0 0; 168 1 0; 148 148 148; 115 115 115; 82 82 82; 49 49 49]./255;  
smoothingwin = 100;
figure; hold on;
for a = 1:length(animals)
    de = [f(a).epochs{1};f(a).epochs{2};f(a).epochs{3}];
    rwdata = [arrayfun(@(x) x.rw',f(a).output{1},'UniformOutput',0),arrayfun(@(x) x.rw',f(a).output{2},'UniformOutput',0), arrayfun(@(x) x.rw',f(a).output{3},'UniformOutput',0)];% stack data from all trials
    rwdata = rwdata(~cellfun(@isempty,rwdata));
    de = de(~cellfun(@isempty,rwdata),:);
...
for e = 1:length(rwdata)
        ...
        days{a}{e} = repmat(de(e,1),length(trigsizes{a}{e}),1);
end
    %subplot(1,2,1); hold on; 
    alltrigs = vertcat(trigsizes{a}{:});
    binnum = floor(length(alltrigs)/5);
    fifths = mean(reshape(alltrigs(1:binnum*5),binnum,5));
    plot([10:20:90],fifths,'.-','Color',animcol(a,:),'LineWidth',2,'MarkerSize',15);
    ylabel('mean trigger size (sd)'); xlabel('Percent of training trials'); xlim([0 100]); ylim([2 16])
        %plot(grpstats(vertcat(trigsizes{a}{:}),vertcat(days{a}{:})),'Color',animcol(a,:))
    %plot(meantrigsize{a},'Color',animcol(a,:))
    %smoothedtrigsize = smooth(padarray(vertcat(trigsizes{a}{:}), smoothingwin, 'replicate'),smoothingwin);
    %smoothedtrigsize = smooth(vertcat(trigsizes{a}{:}),smoothingwin);
    %subplot(1,2,2); hold on; plot(smoothedtrigsize(smoothingwin/2:end-smoothingwin/2),'Color',animcol(a,:)); 
end

%% plot median rate at rw, pre & post, TRIALWISE comparisons
...
for a = 1:length(animals)
    ...
    title('Riprate pre-reward, incltrig, preNF'); xlim([.5 4.5]); ylim([0 1.5]);
    p = ranksum(riprates{a},waitrates{a});
    text(a,1+a/10,sprintf('p=%d\nn=%d,%d trials',p,length(riprates{a}),length(waitrates{a})))
    
    subplot(1,2,2); hold on; title('Rip count pre-reward, incltrig, preNF')
...
end;  title('Outer riprate, rewarded, preNF'); xlim([.5 4.5]); ylim([0 1.5]);
...
for a = 1:length(animals)
    ...
    postrwdata = arrayfun(@(x) x.postrw',f(a).output{1},'UniformOutput',0); % stack data from all trials
    rwdata = rwdata(~cellfun(@isempty,rwdata));
    postrwdata = postrwdata(~cellfun(@isempty,postrwdata));
%     % only use the second half of trials from each epoch
%     rwdata = cellfun(@(x) x(ceil(length(x)/2):end),rwdata,'un',0);
...
xlim([.5 4.5]); ylim([-1 1]);title('rates r, epwise'); plot([.5 4.5],[0 0],'k:')
...
subplot(2,3,2); hold on; title('nums r')
...
subplot(2,3,3); hold on; title('duration r')
...
%% characterize the refractory period AFTER a rip as a function of its size - all ripples (no clear correlation)
clearvars -except f animals ripcols waitcols
figure; set(gcf,'Position',[187 1 1374 973]);
for a = 1:length(animals)
    rwdata = arrayfun(@(x) x.rw,f(a).output{1},'UniformOutput',0); % stack data from all trials
...
for a = 1:length(animals)
    ...
    valbins_rip(a,:) = sum(binclude{a}(type{a}==1,:))>20;
    valbins_wait(a,:) = sum(binclude{a}(type{a}==2,:))>20;
      figure(indivs)  
...
for a = 1:length(animals)
        ...
        for e = 1:length(homedata)
            ...
            sizestack{a}{e}(cellfun(@(x) x.trialnum,homedata{e}),1) = cellfun(@(x) sum(x.size),homedata{e});
            sizestack{a}{e}(cellfun(@(x) x.trialnum,rwdata{e}),2) = cellfun(@(x) sum(x.size),rwdata{e});
            sizestack{a}{e}(cellfun(@(x) x.trialnum,postrwdata{e}),3) = cellfun(@(x) sum(x.size),postrwdata{e});
            sizestack{a}{e}(cellfun(@(x) x.trialnum,outerdata{e}),4) = cellfun(@(x) sum(x.size),outerdata{e});
            sizestack{a}{e}(cellfun(@(x) x.trialnum,lockdata{e}),5) = cellfun(@(x) sum(x.size),lockdata{e});
%             figure; 
